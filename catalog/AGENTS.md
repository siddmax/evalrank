...
- Keep `metric_direction` null in discovery; require explicit `higher` or `lower` from the validated parser contract before `shadow` or `active`. Never infer it from labels or observed values.
