...
- `tests/test_core_contracts.py` checks public capability fingerprint, raw entry, request, candidate set, `the_call`, recommendation ID aliases, ranking, evidence, and methodology-version contract behavior.
- `tests/test_core_fixtures.py` checks reusable public capability fingerprint, raw entry, request, candidate set, recommendation with `the_call`, and evidence fixture payloads.
...
- `tests/test_schema_contracts.py` checks public JSON Schema files against the core payload keys, raw-entry and candidate-set schema shapes, RFC 9457 problem details, structured `the_call`, recommendation alias patterns, and pinned public patterns.
