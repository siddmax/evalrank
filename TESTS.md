...
- `tests/test_core_contracts.py` checks public capability fingerprint, raw entry, request, candidate set, stage candidate, result row, use-case catalog, ranking group, evidence set, exclusion, `the_call`, recommendation ID aliases, recommendation envelope validation, ranking, public string-field validation, primitive/sequence field validation, score-component map validation, evidence metadata, request constraints, and methodology-version contract behavior.
