...
- `tests/test_cli_fixture.py` checks deterministic public CLI fixture output, README fixture-command coverage, invalid input handling, and the `recommend` command for explicit HTTP(S)-only public API success, route path, stdin input, malformed/non-object request JSON, and Problem Details errors.
- `tests/test_mcp_fixture.py` checks the public MCP fixture tool manifest, README fixture-kind coverage, result shape, and explicit HTTP(S)-only recommendation tool behavior with Problem Details errors.
