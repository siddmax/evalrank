...
- `tests/test_mcp_fixture.py` checks the public MCP fixture tool manifest, exact README fixture-kind and tool coverage, result shape, explicit HTTP(S)-only route-tool `base_url` schemas, metadata tools for `GET /v1/use-cases` and `GET /v1/scoring-stages`, and recommendation tool behavior with Problem Details errors.
