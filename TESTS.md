...
- `tests/test_core_contracts.py` checks the independent discovery contracts, pinned use-case taxonomy envelope, strict extension-preserving Problem Details decoding, shared URI vectors, and complete absence of superseded request/recommendation/stage/ranked DTOs.
...
- `tests/test_catalog_manifest.py` checks the canonical 26-cell taxonomy, exact 77-family/79-feed inventory, absence of retired aliases, unique cell/ranking-group/family/feed IDs, preview/discovery/shadow/quarantine truth, explicit native-metric direction for implemented feeds, coherent identity triples, resolved explorer ceilings, exact research-job mappings, multi-feed correlation, fail-closed admission states, independently counted active-family gates, cross-record links, rights/retention completeness, closed schema surface, and exact core-fixture projection.
...
- `tests/test_mcp_fixture.py` checks exact tool discovery, closed decision input, share/receipt semantics, metadata reads, golden receipt parity, and MCP error projection.
...
- `tests/test_sdk_python.py` checks portable re-exports and validates decision/receipt/health clients, strict receipt IDs, canonical request bytes, response hash verification, Problem Details, and absence of compatibility methods.
- `tests/test_sdk_ts.py` checks package metadata, mirrored public constants, launch read/decision types, all seven client paths, semantic verifiers, and absence of retired route vocabulary.
