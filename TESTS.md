...
- `tests/test_core_contracts.py` checks public capability fingerprint, raw entry, request, candidate set, stage candidate, evidence set, exclusion, `the_call`, recommendation ID aliases, ranking, evidence, and methodology-version contract behavior.
- `tests/test_core_fixtures.py` checks reusable public capability fingerprint, raw entry, request, candidate set, stage candidate, evidence set, exclusion, recommendation with `the_call`, and evidence fixture payloads.
