...
- Public/private dirty-worktree refresh confirming current uncommitted private-side changes route to Syndai coding runtime reliability, repo-guidance/doc-validation, and Memphant/memory workstreams, not raw EvalRank public-core ports.
...
| Current Syndai dirty worktree | Uncommitted private edits currently span coding worker/recovery reliability, repo-guidance/doc-validation, and Memphant memory-system planning categories. | Do not copy any of this into EvalRank now. Route coding/runtime work to Syndai, Memphant planning to the memory-system workstream, and EvalRank-named migration/doc-validator material to private input status until a public contract or public doc check is explicitly extracted. | Syndai runtime workstream, Memphant / memory-system workstream, Docs / Public Planning, Open-Core Boundary / CI |
...
| Keep the current Syndai dirty worktree out of EvalRank. | Syndai runtime workstream, Memphant / memory-system, Docs / Public Planning | Current dirty categories are coding worker/recovery reliability, repo-guidance/doc-validation, and Memphant planning. Port only a future explicit storage-free EvalRank contract or public doc check extracted from that work. |
...
| --- | --- | --- |
| Coding worker/recovery and backend runtime reliability edits are currently dirty in Syndai. | Do not port to EvalRank. They are Syndai runtime implementation work, not public EvalRank contracts. | Syndai runtime workstream |
| Repo-guidance and private doc-validation edits are currently dirty in Syndai. | Keep private unless a deterministic public doc check is deliberately extracted and can run only against public docs. | Docs / Public Planning, Open-Core Boundary / CI |
...
| Current Syndai dirty categories: coding worker/recovery reliability, repo-guidance/doc-validation, and Memphant memory-system planning | Keep in their owning private workstreams. They should not be copied into EvalRank; only a future explicit EvalRank contract or public doc check extracted from them belongs here. | Syndai runtime workstream, Memphant / memory-system workstream, Docs / Public Planning, Public Contracts only after extraction |
