...
- Recommendation `RankingGroup.ranked` schemas pin unique grouped ranked rows, matching the public core duplicate-entity guard.
...
| Recommendation comparability and ranking groups | Ported as storage-free `RankingGroup` rows for within-kind rankings only, with schema-level grouped ranked-row uniqueness; cross-kind normalization, scorer internals, and private score semantics stay private. | Public Contracts, Methods / Schemas, SDK / CLI / MCP |
