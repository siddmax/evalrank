...
| Keep the current Syndai dirty worktree out of EvalRank. | Syndai runtime workstream, Memphant / memory-system, Docs / Public Planning | Current dirty categories are preflight/repo guidance, backend runtime reliability, private EvalRank doc-validation, and Memphant planning. Port only a future explicit storage-free EvalRank contract or public doc check extracted from that work. |
...
| --- | --- | --- |
| Preflight and repo-guidance edits are currently dirty in Syndai. | Do not port to EvalRank. They govern the private Syndai ship loop and repo guidance, not the public EvalRank core. | Syndai runtime workstream, Open-Core Boundary / CI only if a public-safe check is extracted |
| Backend runtime reliability edits are currently dirty in Syndai. | Do not port to EvalRank. They are private runtime implementation work, not public EvalRank contracts. | Syndai runtime workstream |
| Private EvalRank doc-validation edits are currently dirty in Syndai. | Keep private unless a deterministic public doc check is deliberately extracted and can run only against public docs. | Docs / Public Planning, Open-Core Boundary / CI |
| Memphant memory-system specs and plans are currently dirty in Syndai. | Do not port to EvalRank. Route to the memory-system workstream unless a future task extracts a concrete EvalRank storage-free payload or route contract. | Memphant / memory-system workstream, outside this repo |
| EvalRank private source areas still exist in Syndai docs, plans, backend migration scripts, and migration directories. | Keep using them only as inputs for sanitized summaries and explicit storage-free public contracts. Do not copy raw private specs, migration scripts, operational plans, or live infrastructure assumptions. | Public Contracts, Public Surface Contracts, DB Bootstrap / Syndai Ops |
...
| Current Syndai dirty categories: preflight/repo guidance, backend runtime reliability, private EvalRank doc-validation, and Memphant memory-system planning | Keep in their owning private workstreams. They should not be copied into EvalRank; only a future explicit EvalRank contract or public doc check extracted from them belongs here. | Syndai runtime workstream, Memphant / memory-system workstream, Docs / Public Planning, Public Contracts only after extraction |
