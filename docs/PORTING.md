...
- TypeScript SDK package metadata and mirrored public contract types/constants, including public fixture kinds, Problem Details codes, abstention, and shape.
...
| SDK / CLI / MCP | Python SDK re-exports plus dependency-free `EvalRankClient` behavior for the public recommendation route; TypeScript public types/constants including fixture kinds and abstention; shared public fixture-kind dispatch; deterministic CLI fixture command; and deterministic MCP fixture adapter, including `raw-entry`, `candidate-set`, `stage-candidate`, `result-row`, `problem`, `use-cases`, `ranking-group`, `evidence-set`, and `exclusion`. | Auth, tenant/project operations, production evidence lookup, source adapters, gate policy, hosted-only workflows, service discovery, retries, and hosted receipt behavior. |
...
| Non-fixture SDK/CLI/MCP behavior for `POST /v1/recommendations` | This repo after client semantics are pinned | SDK / CLI / MCP, Public Surface Contracts | Python SDK stdlib client is now ported; CLI, MCP, TypeScript, retries, auth, tenant context, live hosted receipts, private DTOs, and production service dependencies stay out until separately pinned. |
...
| 3 | Additional non-fixture `POST /v1/recommendations` client semantics. | This repo after each client contract is pinned. | Python SDK HTTP(S)-only success/error JSON handling is ported. Exclude auth, tenants, hosted receipts, live service dependencies, private DTOs, retries, service discovery, local file URLs, and production evidence lookup. |
...
| Extend non-fixture `POST /v1/recommendations` client behavior only after each additional request/response/client error semantic is pinned. | SDK / CLI / MCP, Public Surface Contracts | Python SDK HTTP(S)-only success/error JSON handling is public now; CLI, MCP, TypeScript, retries, auth, tenant context, hosted receipts, private DTOs, service URLs, and production evidence lookup stay out until separately pinned. |
...
| SDK, CLI, and MCP implementations | This repo | SDK / CLI / MCP | Python SDK re-export, TypeScript public types, CLI fixture command, and MCP fixture adapter ported, including candidate-set, stage-candidate, result-row, problem, use-cases, scoring-stages, ranking-group, evidence-set, exclusion, and recommendation abstention-field fixture surfaces |
