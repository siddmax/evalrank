...
| 3 | Additional non-fixture client semantics for currently public routes. | This repo after each client contract is pinned. | Python SDK HTTP(S)-only metadata and recommendation success/error JSON handling, TypeScript native `fetch` recommendation success/error JSON handling, CLI explicit HTTP(S)-only file/stdin command, and MCP explicit tool are ported. Exclude auth, tenants, hosted receipts, live service dependencies, private DTOs, retries, service discovery, environment-variable defaults, local file URLs, and production evidence lookup. |
...
| Extend non-fixture public-route client behavior only after each request/response/client error semantic is pinned. | SDK / CLI / MCP, Public Surface Contracts | Python SDK HTTP(S)-only metadata and recommendation success/error JSON handling, TypeScript native `fetch` recommendation handling, CLI explicit HTTP(S)-only recommendation command, and MCP explicit recommendation tool are public now; retries, auth, tenant context, hosted receipts, private DTOs, service URLs, environment-variable defaults, and production evidence lookup stay out until separately pinned. |
...
| SDK, CLI, and MCP implementations | This repo | SDK / CLI / MCP | Python SDK re-export/client with metadata and recommendation route helpers, TypeScript public types/client, CLI fixture/recommend commands, and MCP fixture/recommend tools ported, including candidate-set, stage-candidate, result-row, problem, use-cases, scoring-stages, ranking-group, evidence-set, exclusion, and recommendation abstention-field fixture surfaces |
