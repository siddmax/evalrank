...
| 3 | Additional non-fixture `POST /v1/recommendations` client semantics. | This repo after each client contract is pinned. | Python SDK HTTP(S)-only success/error JSON handling, TypeScript native `fetch` success/error JSON handling, and CLI explicit HTTP(S)-only file/stdin command are ported. Exclude auth, tenants, hosted receipts, live service dependencies, private DTOs, retries, service discovery, environment-variable defaults, local file URLs, and production evidence lookup. |
...
| Extend non-fixture `POST /v1/recommendations` client behavior only after each additional request/response/client error semantic is pinned. | SDK / CLI / MCP, Public Surface Contracts | Python SDK HTTP(S)-only success/error JSON handling, TypeScript native `fetch` success/error JSON handling, and CLI explicit HTTP(S)-only file/stdin command are public now; MCP, retries, auth, tenant context, hosted receipts, private DTOs, service URLs, environment-variable defaults, and production evidence lookup stay out until separately pinned. |
...
| SDK, CLI, and MCP implementations | This repo | SDK / CLI / MCP | Python SDK re-export/client, TypeScript public types/client, CLI fixture/recommend commands, and MCP fixture adapter ported, including candidate-set, stage-candidate, result-row, problem, use-cases, scoring-stages, ranking-group, evidence-set, exclusion, and recommendation abstention-field fixture surfaces |
