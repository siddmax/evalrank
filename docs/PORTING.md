...
- Exact TESTS abstention terminology drift guard.
...
- Python and TypeScript SDK clients strictly parse public `Retry-After` headers as non-negative integer seconds and treat malformed retry hints as absent while preserving Problem Details errors.
...
- MCP public route tool schemas advertise non-empty HTTP(S) `base_url` inputs.
- MCP recommendation tool schema advertises the public `EvaluationRequest` input shape.
...
- Public scoring-stage catalogs require contiguous `1..N` ordinals in the Python core; scorer formulas, thresholds, graders, telemetry, and runtime behavior stay private/out.
...
- Public recommendation abstention envelopes require `the_call` and `abstention` to agree in core, JSON Schema, and TypeScript types.
- Public recommendation abstentions are pinned to empty single-scale responses in core, JSON Schema, and TypeScript types.
- Public recommendation `shortlist_depth` is pinned in the Python core to the count of returned ranked rows for single-scale and kind-grouped responses.
- Public recommendation ranked rows use contiguous `1..N` rank positions in array order in the Python core for both single-scale recommendations and within-kind `RankingGroup` rows.
- Public kind-grouped recommendation groups are unique by group key and entity type in the Python core, and whole group rows are unique in JSON Schema.
...
- TypeScript SDK `RecommendationCallState` mirrors already-public no-call, recommend-call, and abstain-call response states.
- Scoring-stage catalog schema tests pin stage and contract-ref uniqueness for the already-public catalog shape.
- Recommendation `ranked`, `RankingGroup.ranked`, and `exclusions` schemas pin unique rows, matching the public core duplicate guards.
...
- Earlier public/private porting audit confirming that the then-current private Syndai dirty worktree contained Memphant spec edits and two Memphant plan files, with no EvalRank public-port candidate.
...
- Public/private dirty-worktree refresh confirming current uncommitted private-side changes route to Syndai preflight/repo guidance, backend runtime reliability, private EvalRank doc-validation, and Memphant/memory workstreams, not raw EvalRank public-core ports.
...
| TESTS abstention terminology exact drift guard. | Already ported because it keeps the public test map aligned with the implemented empty single-scale abstention contract name. | Open-Core Boundary / CI, Docs / Public Planning, Public Contracts |
...
| Problem Details public `doc_url` hardening. | Already ported because it aligns the public core and schema around public HTTP(S) documentation links without moving private problem types, hosted internals, telemetry, or route enforcement. | Public Contracts, Public Surface Contracts, Methods / Schemas |
...
| Recommendation abstention envelope consistency. | Already ported as core, JSON Schema, and TypeScript type parity requiring `the_call` and `abstention` to agree; scorer thresholds, evidence floors, private reason taxonomy, runtime, DB, and hosted operations stay private. | Public Contracts, Methods / Schemas, SDK / CLI / MCP |
| Recommendation abstention-as-empty-single-scale behavior. | Already ported as response-shape parity requiring abstentions to carry no ranked rows or groups; scorer thresholds, evidence floors, private reason taxonomy, runtime, DB, and hosted operations stay private. | Public Contracts, Methods / Schemas, SDK / CLI / MCP |
| Recommendation `shortlist_depth` count consistency. | Already ported as Python core response-shape parity requiring the advertised shortlist depth to match single-scale ranked rows or grouped ranked rows. JSON Schema remains structural because array-count arithmetic belongs in the reference contract; scorer thresholds, private ranking policy, runtime, DB, hosted operations, and telemetry stay private. | Public Contracts |
| Recommendation group uniqueness. | Already ported as Python core duplicate group-key/entity-type rejection and JSON Schema whole-row uniqueness for grouped recommendations; cross-kind normalization, scorer internals, runtime, DB, hosted operations, and private score semantics stay private. | Public Contracts, Methods / Schemas |
| Recommendation exclusion uniqueness. | Already ported as Python core and JSON Schema public response-shape hardening for existing storage-free exclusion rows; gate policy, private safety taxonomy, scorer runtime, DB work, hosted operation, and telemetry stay private/out. | Public Contracts, Methods / Schemas |
| Scoring-stage ordinal contiguity. | Already ported as Python core public method-stage order hardening requiring stage ordinals to be contiguous from `1..N`. JSON Schema remains structural; scorer formulas, private methodology, runtime, DB work, telemetry, and hosted behavior stay private/out. | Public Contracts, Methods / Schemas |
...
| SDK `Retry-After` parser hardening. | Already ported as strict non-negative integer parsing for Python and TypeScript clients. Malformed retry hints are ignored while public Problem Details errors are still surfaced; no retry loop, hosted policy, telemetry, or service behavior moved. | SDK / CLI / MCP, Public Surface Contracts |
...
| MCP route tool base URL schemas. | Already ported as input-schema parity for existing public MCP metadata and recommendation tools; the manifests now require non-empty HTTP(S) `base_url` strings without adding service discovery, auth, retries, hosted receipts, private service calls, or database work. | SDK / CLI / MCP, Public Surface Contracts |
| MCP recommendation request schema. | Already ported as input-schema parity for the existing public recommendation route; the tool manifest now advertises the public `EvaluationRequest` fields while private DTOs, auth, hosted receipts, service discovery, database work, and scorer runtime stay out. | SDK / CLI / MCP, Public Surface Contracts |
...
| Current Syndai dirty worktree | Uncommitted private edits currently span preflight/repo guidance, backend runtime reliability, private EvalRank doc-validation, and Memphant memory-system planning categories. | Do not copy any of this into EvalRank now. Route preflight/runtime work to Syndai, Memphant planning to the memory-system workstream, and EvalRank-named doc-validator material to private input status until a public contract or public doc check is explicitly extracted. | Syndai runtime workstream, Memphant / memory-system workstream, Docs / Public Planning, Open-Core Boundary / CI |
...
Rechecked on 2026-06-26 after the latest private-side dirty-worktree scan. The scan found 96 dirty private-side paths by category only; no private file contents were copied here.
...
| Keep the current Syndai dirty worktree out of EvalRank. | Syndai runtime workstream, Memphant / memory-system, DB Bootstrap / Syndai Ops, Hosted Ops / GTM, Docs / Public Planning | Current dirty categories are repo guidance/preflight/test-map docs, backend runtime reliability/orchestration/tools, private backend regression tests, private DB migration/bootstrap, coding runtime/governance docs, private plans/architecture notes, Memphant planning, and mobile/web auth or connection UI. Port only a future explicit storage-free EvalRank contract or public doc check extracted from that work. |
...
Checked and rechecked the private Syndai worktree on 2026-06-26 before this public doc sync. The recheck used path categories and counts only, not private file contents.

| Private-side state | Port decision | Workstream |
| --- | --- | --- |
| 5 repo guidance, preflight, and test-map doc paths are dirty in Syndai. | Do not port wholesale. Extract only public-safe checks or guidance that applies to this Apache-2.0 core repo. | Syndai runtime workstream, Open-Core Boundary / CI only if a public-safe check is extracted |
| 26 backend runtime reliability, orchestration, and tool paths are dirty in Syndai. | Do not port to EvalRank now. These are private runtime implementation paths, not public storage-free contracts. | Syndai runtime workstream, Scoring / Materializer Runtime only after a separable public-input-only slice exists |
| 31 private backend regression-test paths are dirty in Syndai. | Keep with the private runtime work they verify. Distill only public invariants into this repo after matching public docs or contracts exist. | Syndai runtime workstream, Open-Core Boundary / CI only after extraction |
| 1 private DB migration/bootstrap path is dirty in Syndai. | Keep private while EvalRank uses the shared Finn/Supabase bootstrap. Do not copy live migration scripts here. | DB Bootstrap / Syndai Ops |
| 3 coding runtime/governance doc paths are dirty in Syndai. | Keep private unless a product-neutral public contract or public doc check is deliberately extracted. | Syndai runtime workstream, Docs / Public Planning |
| 5 private plan or architecture-note paths are dirty in Syndai. | Use only as private inputs for sanitized summaries. Do not copy raw private plans into this public repo. | Docs / Public Planning |
| 19 Memphant memory-system planning paths are dirty in Syndai. | Do not port to EvalRank. Route to the memory-system workstream unless a future task extracts a concrete EvalRank storage-free payload or route contract. | Memphant / memory-system workstream, outside this repo |
| 5 mobile/web auth or connection UI paths are dirty in Syndai. | Do not port to EvalRank now. These are hosted/private app-surface changes, not public core contracts. | Hosted Ops / GTM, Public Surface Contracts only after a public UI/API contract exists |
| No current uncommitted private artifact should move wholesale into this public repo. | Next public work remains contract hardening, pinned storage-free payloads, public route contracts, and public doc checks, not raw private-doc or runtime-code copying. | Docs / Public Planning, Public Contracts, Public Surface Contracts |
...
| Current Syndai dirty categories: repo guidance/preflight/test-map docs, backend runtime reliability/orchestration/tools, private backend tests, private DB migration/bootstrap, coding runtime/governance docs, private plans, Memphant planning, and mobile/web auth or connection UI | Keep in their owning private workstreams. They should not be copied into EvalRank; only a future explicit EvalRank contract or public doc check extracted from them belongs here. | Syndai runtime workstream, DB Bootstrap / Syndai Ops, Memphant / memory-system workstream, Hosted Ops / GTM, Docs / Public Planning, Public Contracts only after extraction |
...
| Storage-free public payloads and aliases currently represented by core dataclasses and JSON Schemas | Already ported for capability fingerprints, raw entries, evaluation requests, candidate sets, stage candidates, result rows, use-case catalogs, ranking groups, evidence sets, exclusions, `the_call`, abstentions, ranked entities, recommendations, recommendation aliases, strict public string fields, strict public calendar-valid freshness/result-run dates, strict public UTC timestamp format, strict public HTTP(S) result source URLs, strict recommendation envelope validation, strict recommendation rank contiguity/order, strict recommendation group/exclusion uniqueness, strict abstention/the-call consistency, strict abstention-as-empty-single-scale behavior, strict recommendation `shortlist_depth` count consistency, tuple-backed candidate/evidence set validation, recommendation ranked-row schema uniqueness, entity refs, and evidence items. | Public Contracts |
...
| `CandidateSet` candidate-resolution shape | Ported as a storage-free list of public `EntityRef` candidates; Python core inputs must be tuple-backed before serialization to public arrays. Live candidate resolution, source adapters, graph lookup, and production entity rows stay private. | Public Contracts, Methods / Schemas |
...
| `EvidenceSet` evidence-attachment shape | Ported as a storage-free list of public `EvidenceItem` rows; empty tuple-backed inputs support abstention or no-evidence paths before serialization to public arrays. Live evidence lookup, evidence-ledger persistence, source adapters, production traces, and private rows stay private. | Public Contracts, Methods / Schemas |
...
| Public `the_call` / decision-confidence response shape and `Abstention` reason/detail object | Ported as nested recommendation contracts with envelope consistency, empty single-scale abstention behavior, and Python core `shortlist_depth` count consistency, with no proprietary thresholds, held-out evidence floors, private reason taxonomy, or private confidence tuning. | Public Contracts, Methods / Schemas |
...
| Recommendation comparability and ranking groups | Ported as storage-free `ranked` and `RankingGroup` rows for rankings only, with schema-level ranked-row uniqueness; cross-kind normalization, scorer internals, and private score semantics stay private. | Public Contracts, Methods / Schemas, SDK / CLI / MCP |
...
| --- | --- | --- | --- |
| Public contract dataclasses and JSON Schemas | This repo | Public Contracts | Capability fingerprint, raw entry, request, candidate set, stage candidate, result row, use-case catalog, ranking group, evidence set, exclusion, `the_call`, abstention, recommendation, entity, evidence, public calendar-valid freshness/result-run dates, public UTC timestamp format, public HTTP(S) result source URLs, tuple-backed sequence validation, recommendation ranked-row uniqueness, recommendation rank contiguity/order, recommendation exclusion uniqueness, abstention envelope consistency, abstention-as-empty-single-scale, and recommendation shortlist-depth count consistency slices ported |
...
| Public scoring-stage vocabulary, scoring stage catalog, use-case taxonomy method, and method boundaries | This repo | Methods / Schemas | Public boundary notes, contiguous ordinal hardening, and storage-free catalog ported |
...
| SDK, CLI, and MCP implementations | This repo | SDK / CLI / MCP | Python SDK re-export/client with metadata and recommendation route helpers plus strict `Retry-After` parsing, TypeScript public types/client with metadata and recommendation route helpers plus strict `Retry-After` parsing, CLI fixture/metadata/recommend commands, and MCP fixture/metadata/recommend tools ported, including candidate-set, stage-candidate, result-row, problem, use-cases, scoring-stages, ranking-group, evidence-set, exclusion, and recommendation abstention-field fixture surfaces |
