...
| Public `the_call` / decision-confidence response shape and `Abstention` reason/detail object | Ported as nested recommendation contracts with envelope consistency and empty single-scale abstention behavior, with no proprietary thresholds, held-out evidence floors, private reason taxonomy, or private confidence tuning. | Public Contracts, Methods / Schemas |
