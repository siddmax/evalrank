...
- Python SDK package metadata, public core contract re-exports, and dependency-free `EvalRankClient` behavior for the public `POST /v1/recommendations` route.
- Python SDK re-exports for public vocabulary constants shared with core contracts.
- TypeScript SDK package metadata, mirrored public contract types/constants, and dependency-free native `fetch` `EvalRankClient` behavior for the public `POST /v1/recommendations` route.
...
| Non-fixture SDK/CLI/MCP behavior for `POST /v1/recommendations` | This repo after client semantics are pinned | SDK / CLI / MCP, Public Surface Contracts | Python SDK stdlib client, TypeScript native `fetch` client, CLI explicit command, and MCP explicit tool are now ported; retries, auth, tenant context, live hosted receipts, private DTOs, environment-variable defaults, and production service dependencies stay out until separately pinned. |
...
| 3 | Additional non-fixture `POST /v1/recommendations` client semantics. | This repo after each client contract is pinned. | Python SDK HTTP(S)-only success/error JSON handling, TypeScript native `fetch` success/error JSON handling, CLI explicit HTTP(S)-only file/stdin command, and MCP explicit tool are ported. Exclude auth, tenants, hosted receipts, live service dependencies, private DTOs, retries, service discovery, environment-variable defaults, local file URLs, and production evidence lookup. |
...
| Extend non-fixture `POST /v1/recommendations` client behavior only after each additional request/response/client error semantic is pinned. | SDK / CLI / MCP, Public Surface Contracts | Python SDK HTTP(S)-only success/error JSON handling, TypeScript native `fetch` success/error JSON handling, CLI explicit HTTP(S)-only file/stdin command, and MCP explicit tool are public now; retries, auth, tenant context, hosted receipts, private DTOs, service URLs, environment-variable defaults, and production evidence lookup stay out until separately pinned. |
...
| SDK, CLI, and MCP implementations | This repo | SDK / CLI / MCP | Python SDK re-export/client, TypeScript public types/client, CLI fixture/recommend commands, and MCP fixture/recommend tools ported, including candidate-set, stage-candidate, result-row, problem, use-cases, scoring-stages, ranking-group, evidence-set, exclusion, and recommendation abstention-field fixture surfaces |
