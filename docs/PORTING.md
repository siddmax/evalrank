...
- Core Python capability fingerprint, raw entry, evaluation request, candidate set, `the_call`, recommendation, entity reference, and evidence item contracts.
...
- Storage-free capability fingerprints, evaluation requests, candidate sets, entity references, evidence items, public fixtures, and schemas.
...
| --- | --- | --- |
| Public Contracts | `CapabilityFingerprintInput`, `RawEntry`, `EvaluationRequest`, `CandidateSet`, `TheCall`, `RankedEntity`, `Recommendation`, public recommendation ID aliases, `EntityRef`, `EvidenceItem`, constants, and synthetic fixture factories. | Source adapters, graph lookup, storage tables, production entity rows, customer context, private score semantics, scorer thresholds, hosted HMAC derivation. |
| Methods / Schemas | JSON Schemas for public payloads, the pinned public `methodology_version` format, and the public scoring-stage vocabulary including `CandidateSet`. | Proprietary weights, thresholds, held-out eval definitions, benchmark answers, and private ranking experiments. |
| SDK / CLI / MCP | Python SDK re-exports, TypeScript public types/constants, deterministic CLI fixture command, and deterministic MCP fixture adapter, including `raw-entry` and `candidate-set`. | Live service clients, auth, tenant/project operations, production evidence lookup, source adapters, and hosted-only workflows. |
...
| --- | --- | --- | --- |
| Storage-free payload contracts, candidate sets, identifier aliases, and JSON-compatible request/response shapes | `packages/core`, `schemas`, SDK types | Public Contracts | Port when the shape stands alone with synthetic fixtures and schema drift tests. |
...
| Storage-free public payloads and aliases currently represented by core dataclasses and JSON Schemas | Already ported for capability fingerprints, raw entries, evaluation requests, candidate sets, `the_call`, ranked entities, recommendations, recommendation aliases, entity refs, and evidence items. | Public Contracts |
| Public fixture surfaces across core, SDKs, CLI, MCP, and examples | Already ported for deterministic synthetic fixtures only, including candidate-set fixtures. | SDK / CLI / MCP, Examples |
| Public scoring-stage vocabulary | Already ported as a method-boundary note, including `CandidateSet`, without formulas, thresholds, private eval data, or benchmarks. | Methods / Schemas |
...
| --- | --- | --- | --- |
| Public contract dataclasses and JSON Schemas | This repo | Public Contracts | Capability fingerprint, raw entry, request, candidate set, `the_call`, recommendation, entity, and evidence slices ported |
...
| SDK, CLI, and MCP implementations | This repo | SDK / CLI / MCP | Python SDK re-export, TypeScript public types, CLI fixture command, and MCP fixture adapter ported, including candidate-set fixture surfaces |
