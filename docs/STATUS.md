...
| TypeScript SDK | Package metadata, public constants, interfaces for current payload contracts, and dependency-free native `fetch` `EvalRankClient` behavior for `GET /v1/use-cases`, `GET /v1/scoring-stages`, and `POST /v1/recommendations` success plus Problem Details errors, with strict numeric `Retry-After` parsing. | Built JS distribution, published package release flow, and additional non-fixture client behavior beyond the current public metadata and recommendation routes. |
