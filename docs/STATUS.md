...
| Interfaces | Built: Python SDK re-exports plus the first stdlib recommendation client, TypeScript public types/constants, deterministic CLI fixture commands, MCP fixture adapter, runnable public example, and README drift guards. | SDK / CLI / MCP promotes more non-fixture behavior only after public route/client contracts are pinned. |
...
- SDK / CLI / MCP workstream: Python SDK now has the first `POST /v1/recommendations` client; promote CLI/MCP/TypeScript or richer SDK behavior only after each public client contract is pinned.
