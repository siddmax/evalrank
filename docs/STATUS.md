...
- Six MTEB retrieval families expose separate embedding and reranking shadow feeds under one component-configuration ranking group. Their official API parser evidence is represented without claiming publication or retention rights: the repository code is Apache-2.0, while the leaderboard API does not establish a license for third-party score records.
