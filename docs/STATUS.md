...
| 3 | SDK / CLI / MCP | This repo | Python and TypeScript SDK metadata route helpers, Python SDK/TypeScript SDK/CLI/MCP first recommendation clients/tools, and fixture surfaces are ported; richer behavior waits for pinned public contracts. |
...
- SDK / CLI / MCP workstream: Python and TypeScript SDKs now cover the public metadata routes plus the first `POST /v1/recommendations` behavior; CLI and MCP have first recommendation behavior. Promote richer behavior only after each public client contract is pinned.
