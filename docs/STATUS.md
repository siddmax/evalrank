...
- Python SDK package metadata, public core contract re-exports, and dependency-free `EvalRankClient` behavior for `POST /v1/recommendations`.
...
| Interfaces | Built: Python SDK re-exports plus the first stdlib recommendation client, CLI fixture and recommendation commands, TypeScript public types/constants, MCP fixture adapter, runnable public example, and README drift guards. | SDK / CLI / MCP promotes more non-fixture behavior only after public route/client contracts are pinned. |
...
- SDK / CLI / MCP workstream: Python SDK and CLI now have first `POST /v1/recommendations` behavior; promote MCP/TypeScript or richer SDK/CLI behavior only after each public client contract is pinned.
